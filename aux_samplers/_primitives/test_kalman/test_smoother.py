...
def test_parallel_vs_sequential(seed, T, dx, dy, mode):
    ...
    means, covs, samples = sampling_fn(jax_key)

    expected_ms, expected_Ps = explicit_kalman_smoothing(ms, Ps, Fs, Qs, bs)
    npt.assert_allclose(expected_ms, means)
    npt.assert_allclose(expected_Ps, covs)
...
def test_batched_model(seed, T, dx, dy, mode):
    ...
    means, covs, samples = jax.vmap(sampling_fn)(sampling_keys)
    batched_means, batched_covs, batched_samples = jax.vmap(batched_sampling_fn)(sampling_keys)
    npt.assert_allclose(means, batched_means.reshape((-1, T, B * dx)), atol=1e-10, rtol=1e-10)